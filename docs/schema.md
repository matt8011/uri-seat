...
- `recipe_ingredients`: imported per-portion ingredient usage rows for recipes
- `recipes`: derived recipe records rebuilt from `recipe_ingredients` when available
...
Stores imported `recipe + ingredient` rows with the grams of each ingredient in a single serving portion.
...
| --- | --- | --- |
| `id` | `INTEGER` | Primary key, autoincrement |
| `recipe_name` | `TEXT` | Recipe display name |
| `ingredient_name` | `TEXT` | Matches `food_entries.name` case-insensitively |
| `grams_in_portion` | `REAL` | Grams of this ingredient in one serving portion |
| `created_at` | `TEXT` | ISO timestamp |
| `updated_at` | `TEXT` | ISO timestamp |

## Table: `recipes`

Stores recipe-level records generated from `recipe_ingredients` when recipe portion rows exist. If no recipe portion rows exist yet, the app falls back to the older ingredient-tag grouping approach.
...
| `sustainability_index` | `REAL` | Derived recipe score |
...
The `recipes` table is derived data. The admin-only "Repopulate Recipes" action clears the table and rebuilds it using one of two modes:

- preferred mode: join `recipe_ingredients` to `food_entries` by ingredient name, scale each per-100g metric by `grams_in_portion / 100`, sum those ingredient contributions into recipe totals, and then calculate the recipe's nutrition/environmental/sustainability scores
- fallback mode: if `recipe_ingredients` is empty, group `food_entries` by `tagged_recipes` and use the older grouping logic
...
- Recipe rows are not hand-entered; they are regenerated from recipe portion imports, or from ingredient tags when no recipe portion rows exist yet.
